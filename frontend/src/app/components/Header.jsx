...
const Header = () => {
  ...
  // const handleUserClick = async(differentUserId) => {
  //   try {
  //     setLoading(true)
  //     setIsSearchOpen(false)
  //     setSearchQuery("")
  //     await router.push(`user-profile/${differentUserId}`)
  //   } catch (error) {
  //     console.log(error)
  //   }finally{
  //     setLoading(false)
  ...
  const handleUserClick = async (differentUserId) => {
    ...
    if (!differentUserId || typeof differentUserId !== 'string') {
    ...
    try {
      ...
      const currentRoute = pathname; // e.g., '/user-profile/[userId]'
      const currentUserId = user._id; // Extracted from the dynamic route

      if (currentRoute === '/user-profile/[userId]' && currentUserId) {
        if (currentUserId === differentUserId) {
          toast.info("You are already viewing this user's profile.");
          return; // Prevent redundant navigation
        }
        // Navigate to a different user's profile
        await router.push(`/user-profile/${differentUserId}`);
        toast.success("Navigated to the user's profile successfully.");
      } else {
        // Navigate to the user's profile from any other route
        await router.push(`/user-profile/${differentUserId}`);
        toast.success("Navigated to the user's profile successfully.");
      }
    } catch (error) {
      console.error("Navigation Error:", error);
      toast.error("Failed to navigate to the user's profile.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <header className="bg-white dark:bg-[rgb(36,37,38)] text-foreground shadow-md h-16 fixed top-0 left-0 right-0 z-50 p-2">
      <div className="mx-auto flex justify-between items-center p-2">
        ...
        <Button
          variant="ghost"
          size="icon"
          className={`relative flex md:hidden text-gray-600 dark:text-gray-400 hover:text-blue-600 hover:bg-transparent ${activeTab === name ? "text-blue-600" : ""}`}
